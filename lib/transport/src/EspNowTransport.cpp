...
void receiveCallback(const uint8_t *macAddr, const uint8_t *data, int dataLen) {
    ...
    if ((dataLen > header_size) && (dataLen <= MAX_ESP_NOW_PACKET_SIZE) &&
        (memcmp(data, instance->m_buffer, header_size) == 0)) {
        instance->m_output_buffer->add_samples(data + header_size,
                                               dataLen - header_size);
    }
}
...
void EspNowTransport::send() {
    esp_err_t result =
        esp_now_send(broadcastAddress, m_buffer, m_index + m_header_size);
    if (result != ESP_OK) {
